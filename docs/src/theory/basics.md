...
!!! details
	A formal construction of the geometric algebra ``𝔾(V, ⋅)`` over a vector space ``V`` with inner product ``⋅`` is as the [quotient algebra](https://en.wikipedia.org/wiki/Quotient_ring#For_algebras_over_a_ring) `` 𝔾(V, ⋅) ≅ V^⊗ \big/ I ``. Here, the tensor algebra ``V^⊗ = ℝ ⊕ V ⊕ (V ⊗ V) ⊕ ⋯`` is reduced by the [ideal](https://en.wikipedia.org/wiki/Ideal_(ring_theory)) ``I`` which identifies all elements of the form ``𝒖 ⊗ 𝒖 - 𝒖⋅𝒖`` where ``𝒖 ∈ V`` with zero.
...
As a vector space, ``𝔾(V, ⋅)`` is the direct sum of fixed-grade subspaces
```math
...
This operation is at the heart of numerical geometric algebra: multivectors are multiplied by linearly extending it to sums of blades.
...
Not all blades are representable as basis blades in a given choice of basis. For example, in ``𝔾^3`` the product ``𝐯_1(𝐯_2 + 𝐯_3)`` is a blade (since ``𝐯_1`` and ``𝐯_2 + 𝐯_3`` are orthogonal vectors), but it is a sum of two basis blades, ``𝐯_1𝐯_2 + 𝐯_1𝐯_3``.
